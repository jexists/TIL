...
class MarkerOverlappingRecognizer {
	constructor(opts) {
	...
	_onOver(e) {
		...
		if (overlaped.length > 1) {
			if (this._options.highlightRect) {
				...
				for (let i = 0, ii = overlaped.length; i < ii; i++) {
					if (bounds) {
	...
	_renderIntersectList(overlaped, offset) {
		...
		let listLayer = this._overlapListEl;

		let REPEAT_REGEX = /\{\{#repeat\}\}([\s\S]*)\{\{\/#repeat\}\}/gm,
	...
	_onClickItem(marker, e) {
		...
		// marker.trigger('click'); //ERROR 이유 모르겠음
	}
};